...
JULIA_EXT_VER=1.79.2
PYTHON_EXT_VER=2024.4.1
JUPYTER_EXT_VER=2024.4.0
...
chmod +x "$EXT_DIR/ms-vscode.cpptools-${CPPTOOLS_VERSION}/bin/libc.so"
...
grep -q "const timeoutInterval = 120000" "$PWD/lib/code-server/out/node/wrapper.js" || echo "Timeout interval fix was not applied!"

# Jupyter extension assumes writeable installation directory for Jupyter kernel temporary files
sed -i 's/this.context.extensionUri,"temp"/this.platformService.homeDir,".cache","temp"/' "$PWD/lib/code-server/lib/vscode/extensions/ms-toolsai.jupyter-$JUPYTER_EXT_VER-universal/dist/extension.node.js"
grep -q 'this.platformService.homeDir,".cache","temp"' "$PWD/lib/code-server/lib/vscode/extensions/ms-toolsai.jupyter-$JUPYTER_EXT_VER-universal/dist/extension.node.js" || echo "jupyter kernel temp file fix not applied!"
